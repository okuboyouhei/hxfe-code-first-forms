...
**HXFE** は **htmx Form Engine** の頭文字です。フロントエンドには [htmx](https://htmx.org/) を使っています。htmxはHTMLの属性だけでAJAX通信を記述できる軽量ライブラリで、Reactのようなビルド環境が不要です。ページ遷移なしの「入力→確認→完了」フローも、JavaScriptをほぼ書かずに実現しています。
...
| AIとの連携 | ❌ AIが直接編集できない | ✅ AIがコードを直接生成・修正できる |
...
- AIコーディングツール（Claude / Cursor / Copilot など）と一緒に開発したい

### HXFEに向いていない場面

- **PHPが書けない方には向きません。** フォームの定義はすべてPHPコードで行います。GUIでフォームを作りたい場合は CF7・WPForms をお使いください。
- フォームの送信データをWordPressの管理画面で確認したい → Gravity Forms を使う
- フォームを非エンジニアのスタッフが日常的に変更する運用 → CF7・WPForms を使う
...
**方法①：WordPress管理画面から検索（推奨）**

1. WordPress管理画面 → **プラグイン → 新規追加**
2. 検索ボックスに `HXFE` と入力
3. 「HXFE — Code-First Forms」を見つけて **今すぐインストール** → **有効化**

**方法②：ZIPファイルからインストール**

1. [WordPress.org](https://wordpress.org/plugins/hxfe-code-first-forms/) からZIPをダウンロード
...
```php
[ 'key' => 'tel', 'type' => 'tel', 'label' => '電話番号', 'placeholder' => '例: 090-1234-5678' ]
```
...
```php
[ 'key' => 'website', 'type' => 'url', 'label' => 'WebサイトURL', 'placeholder' => 'https://' ]
```
...
[
    ...
    'type'     => 'date',
    'label'    => '希望日',
    'min_date' => date('Y-m-d'),         // 今日以降のみ選択可
    'max_date' => date('Y-m-d', strtotime('+3 months')),  // 3ヶ月後まで
]
```
...
[
    ...
    'type'        => 'file',
    'label'       => '添付ファイル',
    'accept'      => '.pdf,.doc,.docx',  // 許可する拡張子
    'max_size_mb' => 5,                   // 最大ファイルサイズ（MB）
]
```

> アップロードされたファイルは管理者宛メールに添付され、送信後に自動削除されます。データベースには保存されません。

---

### cascade — 連動プルダウン

選択した値に応じて次のプルダウンの選択肢が変わります。

```php
// 都道府県を選ぶと市区町村が絞り込まれる例
[
    'key'     => 'prefecture',
    'type'    => 'select',
    'label'   => '都道府県',
...
[
    ...
    // 'hide_if'   => [ ... ],           // 廃止（後方互換あり）→ show_if を使うこと
...
- **追加**: エラーログ機能 — SMTPエラー・Webhookエラー・reCAPTCHAエラーを `wp-content/hxfe-logs/` に記録
- **追加**: 設定 → Form Engine — Logs ページ（管理画面でエラーログを確認・クリア可能）
- **追加**: ログは色分け表示（SMTP / Webhook / reCAPTCHA / File）
- **追加**: ログは30日で自動削除
- **追加**: ログディレクトリに `.htaccess` を自動生成（Webアクセスをブロック）

---

## v1.3.9

- **修正**: htmxのスクリプトハンドル名を `htmx` → `hxfe-htmx` に変更（他プラグインとの競合防止）
- **修正**: 同一ページに同じIDのショートコードを2つ置いた場合、2つ目をHTMLコメントに変換（DOM重複防止）
- **ドキュメント**: lint警告一覧をマニュアルに追加

---

## v1.3.8

- **セキュリティ**: `hxfe_get_client_ip()` をデフォルトで `REMOTE_ADDR` のみ使用に変更（IPスプーフィング防止）
- **セキュリティ**: パスワード比較を `hash_equals()` に変更（タイミング攻撃対策）
- **セキュリティ**: reCAPTCHAのシークレットキー未設定時、本番環境では送信をブロック（fail-closed）
- **改善**: reCAPTCHAキー未設定のスキーマに管理画面で警告を表示

---

## v1.3.7

- **セキュリティ**: `skip_sanitize` パラメータを完全削除。全フローで常にサニタイズを実行
- **改善**: `iframe.php` にREVIEWER NOTEを追加（wp_enqueue_*が使えない理由を明記）

---

## v1.3.6

- **セキュリティ**: `hxfe_handle_back()` にnonce検証を追加
- **修正**: Stable Tagの不一致を解消

---

## v1.3.5

- **変更**: スキーマキー名の整理（後方互換あり）
  - `redirect_rules` → `complete_redirect_rules`
  - `before_open_html` → `before_html` / `after_close_html` → `after_html`
  - `min_check` / `max_check` → `min` / `max`
  - `hide_if` 廃止（`show_if` を使用すること）
...
**設定 → Form Engine — Logs** からエラーログを確認できます。

### ログに記録されるエラー

| エラー種別 | 内容 |
|---|---|
| `SMTP_ERROR` | メール送信失敗（SMTPの設定ミス・タイムアウト等） |
| `WEBHOOK_ERROR` | Webhook送信失敗（URLが無効・HTTP 4xx/5xx等） |
| `RECAPTCHA_ERROR` | reCAPTCHA検証失敗（スコア不足・API接続失敗等） |
| `FILE_ERROR` | ファイルアップロードエラー |
...
- `.htaccess` でWebからのアクセスをブロックしています

### よくある原因と対処法

**SMTPエラーが続く場合：**
設定 → Form Engine → SMTP Settings でSMTP設定を確認してください。Gmailの場合はアプリパスワードを使用してください。

**Webhookエラーが続く場合：**
スキーマの `webhooks[].url` が正しいか確認してください。Zapier/MakeのWebhook URLは期限切れになることがあります。

**reCAPTCHAエラーが続く場合：**
設定 → Form Engine でシークレットキーとサイトキーが正しく設定されているか確認してください。
...
### Q: functions.phpに書いたのにフォームが表示されない
...
### Q: スキーマを functions.php 以外のファイルで管理できますか？
...
### Q: 管理画面からPHPファイルを直接編集できないようにしたい
